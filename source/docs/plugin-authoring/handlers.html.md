...
`:restrict_to` | `Symbol`, `String`, `Array<String, Symbol>` | Authorization groups necessary to trigger the route. The user sending the message must be a member of at least one of the supplied groups. See [authorization groups](/docs/getting-started/usage/#authorization-groups) for more information. | `nil`
...
For more detailed examples, check out the built in authorization, help, and web handlers, or any of the many existing handler plugins on the [plugins](/plugins) page. Refer to Lita's [API documentation](http://rdoc.info/gems/lita) for the exact specifications of handlers' methods.
